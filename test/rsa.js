...
describe('JWT Builder', function () {
  describe('when RS256 is specified', function () {
    var token = new nJwt.Jwt({}).setSigningAlgorithm('RS256').setSigningKey(pair.private).compact();
    var verifier = new nJwt.Verifier().setSigningAlgorithm('RS256').setSigningKey(pair.public);
    var result;
