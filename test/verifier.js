...
describe('.verify()', function () {
  it('should persist the original token to the toString() invocation', function () {
    var token =
      'eyJhbGciOiJub25lIn0.eyJzdWIiOiIxMjMifQ.p6bizskaJLAheVyRhQEMR-60PkH_jtLVYgMy1qTjCoc';
  ...
  describe('if given only a token', function () {
    it('should verify tokens that are alg none', function () {
      ...
      var token = new nJwt.Jwt(claims).setSigningAlgorithm('none').compact();
    ...
    it('should reject tokens that specify an alg', function () {
      var claims = { hello: uuid() };
      var key = uuid();
      var token = new nJwt.create(claims, key).compact();
      assert.throws(function () {
        nJwt.verify(token);
...
describe('Verifier().verify() ', function () {
  it('should support sync usage', function () {
    var verifier = new nJwt.Verifier().setSigningAlgorithm('none');
    var claims = { hello: uuid() };
  ...
  it('should return the jwt string, header and body on error objects', function (done) {
    var jwt = new nJwt.Jwt({ expiredToken: uuid() }).setExpiration(new Date().getTime() - 1000);
  ...
  it('should return the jwt string, header and body on error objects with not active message', function (done) {
    var jwt = new nJwt.Jwt({ notActiveToken: uuid() }).setNotBefore(new Date().getTime() + 1000);
    var token = jwt.compact();
    nJwt.verify(token, function (err) {
      assert.equal(err.jwtString, token);
      assert.equal(err.parsedHeader.alg, jwt.header.alg);
  ...
  describe('when configured to expect no verification', function () {
    ...
    describe('and given an expired token', function () {
      var result;
      var jwt = new nJwt.Jwt({ expiredToken: 'x' }).setExpiration(new Date().getTime() - 1000);
    ...
    describe('and given a not active token', function () {
      var result;
      var jwt = new nJwt.Jwt({ notActiveToken: 'x' }).setNotBefore(new Date().getTime() + 1000);
  ...
  describe('when configured to expect signature verification', function () {
    var verifier = new nJwt.Verifier().setSigningAlgorithm('HS256').setSigningKey('hello');
  ...
  describe('when configured to expect signature verification', function () {
    ...
    var verifier = new nJwt.Verifier().setSigningAlgorithm('HS256').setSigningKey(key);

    var claims = { hello: uuid() };

    describe('and given a token that was signed with the same key', function () {
      var result;
      var token = new nJwt.Jwt(claims).setSigningAlgorithm('HS256').setSigningKey(key).compact();
  ...
  describe('when verifying an invalid ECDSA token', function () {
    ...
    var invalidToken =
      'eyJhbGciOiJFUzUxMiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWV9.82wXTCDa4VHEAaDlq7PyqOqNbMGwDiXSt_n1nKGH43w';

    before(function (done) {
      var verifier = new nJwt.Verifier().setSigningAlgorithm('ES512').setSigningKey(ecdsaPublicKey);
